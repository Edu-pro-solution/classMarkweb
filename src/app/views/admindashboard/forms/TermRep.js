...
const { data } = useFetch(
  sessionId ? `/get-students/${studentId}/${sessionId}` : null
);
...
//     // ✅ REMOVE the examId filter — backend already returns only Second Term
//     const scores = response.data.scores;

//     if (!scores || scores.length === 0) {
//       console.warn("No scores found for this student");
//       return [];
//     }
...
//           return { ...score, position: studentPosition };
//         } catch (e) {
//           return { ...score, position: "-" };
//         }
//       })
...
const fetchStudentData = async (studentId) => {
  try {
    ...
    // ✅ Step 1: Get the correct Second Term examId for this session
    const examRes = await axios.get(
      `${apiUrl}/api/getofflineexam/${currentSession._id}`,
      { headers }
    );
    const exams = examRes.data;
    const secondTermExam = exams.find(e => 
      e.name?.toLowerCase().includes("second term")
    );

    if (!secondTermExam) {
      console.warn("Second term exam not found");
      return [];
    }

    // ✅ Step 2: Fetch scores using the correct examId
    const response = await axios.get(
      `${apiUrl}/api/get-scores-by-student/${studentId}/${currentSession._id}/${secondTermExam._id}`,
      { headers }
    );
    ...
    // Calculate positions (keep your existing code)
    const scoresWithPositions = await Promise.all(
      scores.map(async (score) => {
        const { examId, subjectId } = score;
        if (!examId || !subjectId) return { ...score, position: "-" };
  ...
  const fetchAllStudentsData = async (examId, subjectId, sessionId) => {
  try {
    const token = localStorage.getItem("jwtToken");
    if (!token) {
      throw new Error("JWT token not found");
    ...
    if (data && data.scores && Array.isArray(data.scores)) {
      ...
      console.warn("No scores data available for this subject/exam/session.");
      return [];
    }
  } catch (error) {
    console.error("Error fetching all students data:", error);
    ...
    throw new Error(
      `Failed to fetch all students data for examId=${examId}, subjectId=${subjectId}, sessionId=${sessionId}`
    );
  }
};
