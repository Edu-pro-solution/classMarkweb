 ...
 *  - Prints on a single A4 page (custom pageStyle + tighter print: spacing)
 *  - Fully responsive on mobile (grids stack, tables scroll horizontally)
 */
import { useContext, useEffect, useRef, useState } from "react";
import { useParams, Link } from "react-router-dom";
import axios from "axios";
...
const RatingRow = ({ label, value, max = 5 }: { label: string; value?: number; max?: number }) => (
  <tr className="border-b border-gray-100 last:border-0">
    <td className="py-0.5 px-2 text-[10px] print:text-[8px] font-medium">{label}</td>
    {Array.from({ length: max }, (_, i) => i + 1).map((n) => (
      <td key={n} className="py-0.5 px-1 text-center text-[10px] print:text-[8px] w-6">
        {value === n ? "✓" : ""}
      </td>
    ))}
  </tr>
...
// CSS injected only for the print media — keeps everything to one A4 page
const PRINT_STYLE = `
...
export default function ReportCard({ termLabel }: Props) {
  ...
  // pageStyle forces one clean A4 page instead of the browser's default
  // pagination, which was overflowing onto a second page.
  ...
  return (
    <div className="p-3 sm:p-6 space-y-4 max-w-5xl mx-auto">
      ...
      <div
        ref={printRef}
        className="printable-area bg-white border border-border rounded-lg p-3 sm:p-6 print:p-2 print:border-none space-y-3 print:space-y-1.5 text-black text-sm print:text-[10px]"
      >

        {/* ── SCHOOL HEADER ── */}
        <div className="text-center border-b-2 border-primary pb-2 print:pb-1 space-y-0.5">
          {school.schoolLogo && (
            <img src={school.schoolLogo} alt="School Logo" className="h-16 w-16 print:h-10 print:w-10 object-contain mx-auto mb-1" />
          )}
          <h1 className="text-lg sm:text-xl print:text-base font-black text-primary uppercase tracking-wide">
            {school.name || "School Name"}
          </h1>
          {school.address && <p className="text-[11px] print:text-[9px] text-gray-600">{school.address}</p>}
          {school.motto   && <p className="text-[11px] print:text-[9px] italic text-gray-500">Motto: {school.motto}</p>}
          {school.email   && <p className="text-[11px] print:text-[9px] text-gray-500">{school.email}</p>}
          {school.phone   && <p className="text-[11px] print:text-[9px] text-gray-500">Tel: {school.phone}</p>}
          <div className="mt-1.5 inline-block bg-primary text-white text-xs print:text-[9px] font-bold px-5 py-1 print:py-0.5 rounded-full tracking-wider">
            {currentSession?.name} — {termLabel.toUpperCase()} REPORT CARD
          </div>
        </div>

        {/* ── TOP INFO GRID (3 columns, stacks on mobile) ── */}
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-0 border border-gray-300 rounded-md overflow-hidden text-[11px] print:text-[9px]">

          {/* Col 1: Personal Data */}
          <div className="border-b sm:border-b-0 sm:border-r border-gray-300 p-3 print:p-1.5 space-y-1.5 print:space-y-0.5">
            <p className="font-black text-primary text-[10px] print:text-[8px] uppercase tracking-wider mb-2 print:mb-1">
              Student's Personal Data
            </p>
            {(
              [
          ...
          {/* Col 2: Attendance + Term Dates */}
          <div className="border-b sm:border-b-0 sm:border-r border-gray-300 p-3 print:p-1.5 space-y-2 print:space-y-1">
            <p className="font-black text-primary text-[10px] print:text-[8px] uppercase tracking-wider mb-1">Attendance</p>
            <div className="overflow-x-auto">
              <table className="w-full border border-gray-300 text-center text-[10px] print:text-[8px]">
                <thead className="bg-primary/10">
                  <tr>
                    {["School Opened","Present","Absent"].map(h => (
                      <th key={h} className="border border-gray-300 px-1 py-1">{h}</th>
                ...
                <tbody>
                  <tr>
                    {["—","—","—"].map((v, i) => (
                      <td key={i} className="border border-gray-300 px-1 py-2 print:py-1 font-bold">{v}</td>
                    ))}
                  </tr>
                </tbody>
              </table>
            ...
            <p className="font-black text-primary text-[10px] print:text-[8px] uppercase tracking-wider mt-2 print:mt-1 mb-1">
              Terminal Duration
            </p>
            <div className="overflow-x-auto">
              <table className="w-full border border-gray-300 text-center text-[10px] print:text-[8px]">
                ...
                <tbody>
                  <tr>
                    {[profile.termBegins, profile.termEnds, profile.nextTermBegins || profile.resumptionDate].map((d, i) => (
                      <td key={i} className="border border-gray-300 px-1 py-2 print:py-1">
                        {d ? new Date(d).toLocaleDateString() : "—"}
                      </td>
                    ))}
                  </tr>
          ...
          <div className="p-3 print:p-1.5">
            <p className="font-black text-primary text-[10px] print:text-[8px] uppercase tracking-wider mb-2 print:mb-1">Score Summary</p>
            <div className="space-y-1.5 print:space-y-0.5">
              {(
                ...
                <div key={l} className="flex justify-between items-center border-b border-gray-100 pb-0.5">
                  <span className="text-gray-600 font-semibold">{l}</span>
                  <span className="font-black text-primary text-[12px] print:text-[9px]">{v}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        {/* ── ACADEMIC PERFORMANCE ── */}
        <div>
          <p className="text-center font-black text-primary text-[10px] print:text-[8px] uppercase tracking-widest mb-1">
            Academic Performance
          </p>
          <div className="overflow-x-auto">
            <table className="w-full text-[11px] print:text-[9px] border-collapse border border-gray-300 min-w-[480px]">
              <thead>
                <tr className="bg-primary text-white">
                  {tableHeaders.map((h) => (
                    <th key={h} className="border border-gray-400 px-2 py-1.5 print:py-0.5 text-left whitespace-nowrap font-bold">
                      {h}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                {!hasRows ? (
                  ...
                  cumulativeRows.map((row, i) => (
                    <tr key={i} className={i % 2 === 0 ? "bg-gray-50" : "bg-white"}>
                      <td className="border border-gray-200 px-2 py-1 print:py-0.5 font-medium">{row.subject}</td>
                      <td className="border border-gray-200 px-2 py-1 print:py-0.5 text-center">{row.first  || "—"}</td>
                      <td className="border border-gray-200 px-2 py-1 print:py-0.5 text-center">{row.second || "—"}</td>
                      <td className="border border-gray-200 px-2 py-1 print:py-0.5 text-center">{row.third  || "—"}</td>
                      <td className="border border-gray-200 px-2 py-1 print:py-0.5 text-center font-bold">{row.total}</td>
                      <td className="border border-gray-200 px-2 py-1 print:py-0.5 text-center font-bold">{row.average}</td>
                      <td className={`border border-gray-200 px-2 py-1 print:py-0.5 text-center ${gradeColor(row.grade)}`}>{row.grade}</td>
                      <td className="border border-gray-200 px-2 py-1 print:py-0.5 text-center">{row.remark}</td>
                    </tr>
                  ))
                ) : (
                  scores.map((s, i) => {
                    ...
                    return (
                      <tr key={i} className={i % 2 === 0 ? "bg-gray-50" : "bg-white"}>
                        <td className="border border-gray-200 px-2 py-1 print:py-0.5 font-medium">{s.subjectName || s.subject || "—"}</td>
                        <td className="border border-gray-200 px-2 py-1 print:py-0.5 text-center">{test}</td>
                        <td className="border border-gray-200 px-2 py-1 print:py-0.5 text-center">{exam}</td>
                        <td className="border border-gray-200 px-2 py-1 print:py-0.5 text-center font-bold">{total}</td>
                        <td className={`border border-gray-200 px-2 py-1 print:py-0.5 text-center ${gradeColor(s.grade || grade)}`}>
                          {s.grade || grade}
                        </td>
                        <td className="border border-gray-200 px-2 py-1 print:py-0.5 text-center">{s.comment || comment}</td>
                      </tr>
                    );
                  })
                )}
        ...
        <div>
          ...
          <div className="grid grid-cols-4 sm:grid-cols-7 border border-gray-300 rounded overflow-hidden text-[10px] print:text-[8px]">
            {GRADE_KEYS.map(({ range, label, grade }) => (
              <div key={grade} className="border-r last:border-r-0 border-b sm:border-b-0 border-gray-300 text-center px-1 py-1.5 print:py-0.5 bg-white">
                <p className={`font-black text-[11px] print:text-[9px] ${gradeColor(grade)}`}>{grade}</p>
                <p className="font-semibold text-gray-700">{range}</p>
                <p className="text-gray-500 text-[9px] print:text-[7px]">{label}</p>
              </div>
            ))}
          </div>
        </div>

        {/* ── AFFECTIVE + PSYCHOMOTOR (side by side on desktop, stacked on mobile) ── */}
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 print:gap-2">

          {/* Affective Traits */}
          <div className="border border-gray-300 rounded overflow-hidden">
            <div className="bg-primary text-white text-center font-bold text-[10px] print:text-[8px] py-1.5 print:py-0.5 uppercase tracking-wider">
              Affective Traits
            </div>
            <div className="overflow-x-auto">
              <table className="w-full min-w-[260px]">
                <thead>
                  <tr className="bg-primary/10 border-b border-gray-200">
                    <th className="py-1 px-2 text-left text-[10px] print:text-[8px]">Trait</th>
                    {[1,2,3,4,5].map(n => (
                      <th key={n} className="py-1 px-1 text-center text-[10px] print:text-[8px] w-6">{n}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
          ...
          <div className="border border-gray-300 rounded overflow-hidden flex flex-col">
            ...
            <div className="overflow-x-auto">
              <table className="w-full min-w-[260px]">
                <thead>
                  <tr className="bg-primary/10 border-b border-gray-200">
                    <th className="py-1 px-2 text-left text-[10px] print:text-[8px]">Skill</th>
                    {[1,2,3,4,5].map(n => (
                      <th key={n} className="py-1 px-1 text-center text-[10px] print:text-[8px] w-6">{n}</th>
                    ))}
                  </tr>
            ...
            {/* Rating key legend */}
            <div className="border-t border-gray-200 p-2 print:p-1 text-[10px] print:text-[8px] bg-gray-50 space-y-0.5 flex-1">
              <p className="font-bold text-primary uppercase tracking-wider mb-1">Keys to Rating</p>
              {["1 – Very Poor","2 – Poor","3 – Fair","4 – Good","5 – Excellent"].map(k => (
                <p key={k} className="text-gray-700">{k}</p>
              ))}
