...
// ─── Field definitions (kept in sync with ReportCard.tsx's PsyData) ─────────

const AFFECTIVE_FIELDS = [
  { key: "attentiveness", label: "Attentiveness" },
  { key: "attitudeToWork", label: "Attitude to Work" },
  { key: "cooperation", label: "Cooperation" },
  { key: "emotionStability", label: "Emotional Stability" },
  ...
  { key: "speakingWriting", label: "Speaking / Writing" },
  { key: "organisationAbility", label: "Organisation Ability" },
  { key: "relationshipWithOthers", label: "Relationship w/ Others" },
] as const;
...
const PSYCHOMOTOR_FIELDS = [
  ...
  { key: "retentiveness", label: "Retentiveness" },
] as const;

type AffectiveKey = (typeof AFFECTIVE_FIELDS)[number]["key"];
type PsychomotorKey = (typeof PSYCHOMOTOR_FIELDS)[number]["key"];
type RatingKey = AffectiveKey | PsychomotorKey;
...
function PsychoStudentReport() {
  ...
  const emptyRatings = () => {
    ...
    [...AFFECTIVE_FIELDS, ...PSYCHOMOTOR_FIELDS].forEach(({ key }) => {
      r[key] = 0;
    });
    return r;
  };
        ...
        <div className="space-y-4">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              Rate each trait 1 (Very Poor) – 5 (Excellent). Scroll right to see all columns.
            </p>
          </div>
            ...
            <div className="overflow-x-auto">
              <Table>
                <TableHeader className="bg-[#E8EBF3]">
                  {/* Group header row */}
                  <TableRow>
                    <TableHead rowSpan={2} className="w-[40px] text-[#004aaa] font-bold pl-4 align-bottom">#</TableHead>
                    <TableHead rowSpan={2} className="text-[#004aaa] font-bold align-bottom">Adm No</TableHead>
                    <TableHead rowSpan={2} className="text-[#004aaa] font-bold align-bottom">Name</TableHead>
                    <TableHead colSpan={AFFECTIVE_FIELDS.length} className="text-[#004aaa] font-bold text-center border-l border-white">
                      Affective Traits
                    </TableHead>
                    <TableHead colSpan={PSYCHOMOTOR_FIELDS.length} className="text-[#004aaa] font-bold text-center border-l border-white">
                      Psychomotor Skills
                    </TableHead>
                    <TableHead rowSpan={2} className="text-[#004aaa] font-bold text-[11px] leading-tight min-w-[140px] align-bottom border-l border-white">Teacher Remarks</TableHead>
                    <TableHead rowSpan={2} className="text-[#004aaa] font-bold text-[11px] leading-tight min-w-[140px] align-bottom">Principal Remarks</TableHead>
                  </TableRow>
                  {/* Individual trait header row */}
                  <TableRow>
                    {AFFECTIVE_FIELDS.map(({ key, label }, i) => (
                      <TableHead key={key} className={`text-[#004aaa] font-bold text-[10px] leading-tight text-center w-16 ${i === 0 ? "border-l border-white" : ""}`}>
                        {label}
                      </TableHead>
                    ))}
                    {PSYCHOMOTOR_FIELDS.map(({ key, label }, i) => (
                      <TableHead key={key} className={`text-[#004aaa] font-bold text-[10px] leading-tight text-center w-16 ${i === 0 ? "border-l border-white" : ""}`}>
                        {label}
                      </TableHead>
                    ))}
                ...
                <TableBody>
                  {currentRows.length === 0 ? (
                    <TableRow>
                      <TableCell colSpan={3 + AFFECTIVE_FIELDS.length + PSYCHOMOTOR_FIELDS.length + 2} className="text-center py-10 text-slate-500">
                        No students found.
                      </TableCell>
                    </TableRow>
                  ) : (
                          ...
                          {AFFECTIVE_FIELDS.map(({ key }) => (
                            <TableCell key={key}>
                              <Input
                                ...
                                max={5}
                                value={row[key]}
                                onChange={(e) => handleChange(globalIdx, key, Number(e.target.value))}
                                className="w-14 h-10 text-center border-slate-300"
                              />
                            </TableCell>
                          ))}
                          {PSYCHOMOTOR_FIELDS.map(({ key }) => (
                            <TableCell key={key}>
                              <Input
                                type="number"
                                min={0}
